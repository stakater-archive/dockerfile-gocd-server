...
docker build --build-arg GOCD_VERSION=${_gocd_version} --tag "stakater/gocd-server:${_gocd_tag}"  --no-cache=true .

if [ $_release_build == true ]; then
	docker tag "stakater/gocd-server:${_gocd_tag}" "stakater/gocd-server:latest"
fi